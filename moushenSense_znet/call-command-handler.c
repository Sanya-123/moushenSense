...
EmberAfStatus emberAfClusterSpecificCommandParse(EmberAfClusterCommand *cmd)
{
  ...
  if (cmd->direction == (uint8_t)ZCL_DIRECTION_SERVER_TO_CLIENT
    ...
    case ZCL_IDENTIFY_CLUSTER_ID:
      ...
      break;
    case ZCL_SAMPLE_MFG_SFECIFIC_CIAMMER_ID:
      result = status(false, true, cmd->mfgSpecific);
      break;
    default:
    ...
    case ZCL_IDENTIFY_CLUSTER_ID:
      ...
      break;
    case ZCL_SAMPLE_MFG_SFECIFIC_CIAMMER_ID:
      result = emberAfSampleMfgSfecificCiammerServerCommandParse(cmd);
      break;
    default:
      // Unrecognized cluster ID, error status will apply.
      break;
...
// Cluster: Sample Mfq Specific Cluster, server
EmberAfStatus emberAfSampleMfgSfecificCiammerServerCommandParse(EmberAfClusterCommand *cmd)
{
  bool wasHandled = false;
  if (!cmd->mfgSpecific) {
    switch (cmd->commandId) {
    case ZCL_SET_POEPLE_INCREMENT_COMMAND_ID:
      {
        // Command is fixed length: 0
        wasHandled = emberAfSampleMfgSfecificCiammerSetPoepleIncrementCallback();
        break;
      }
    case ZCL_SET_POEPLE_DINCREMENT_COMMAND_ID:
      {
        // Command is fixed length: 0
        wasHandled = emberAfSampleMfgSfecificCiammerSetPoepleDincrementCallback();
        break;
      }
    case ZCL_SET_POEPLE_ZERO_COMMAND_ID:
      {
        // Command is fixed length: 0
        wasHandled = emberAfSampleMfgSfecificCiammerSetPoepleZeroCallback();
        break;
      }
    default:
      {
